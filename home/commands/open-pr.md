...
session context — see [`~/protocols/HANDOFF-PROTOCOL.md`](../protocols/HANDOFF-PROTOCOL.md) for the
...
- Print the title + body as a single **preview block** — for the record, not for
  ...
  **print-only, AUTO gate in both modes** (per
  [`~/protocols/HANDOFF-PROTOCOL.md`](../protocols/HANDOFF-PROTOCOL.md)'s taxonomy): never STOP for `go`
