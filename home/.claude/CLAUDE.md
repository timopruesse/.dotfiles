...
| --- | --- | --- | --- |
| cheap | `boba-watcher`, `committer`, `researcher`, `scout` | `haiku` | `kimi-k2.7-code` |
...
`~/.cursor/rules/subagent-model-fallback.mdc`). Hard must-nots also ship as
`~/.cursor/rules/agent-routing.mdc` (generated).

<!-- BEGIN GENERATED AGENT ROUTING -->
# Agent routing (hard)

Pinned agents: `boba-watcher`, `committer`, `researcher`, `scout`, `pr-babysitter`, `pr-reviewer`, `scout-explain`, `sweep`, `worker`, `verifier`.
...
- **`scout`** (cheap) — read-only LOCATE agent: pinpoint "where/how does X work"
  ...
  strong / orchestrator model. **Hard rule:** never use Explore/generalPurpose
  for locate — spawn `scout` by name.
...
- **`researcher`** (cheap) — read-only RESEARCH / spike-prep: external + undecided
  context (web, `gh`, ticket comments) → ranked hypotheses and open questions.
  Opt-in before `/dispatch` for research/spike tickets (`/open-work` may offer
  it). Not locate, not codebase explain. Ends `ADVANCE → parent` or `HALT:`.
- **`sweep`** — mechanical fix loops (tsc/type errors, lint, formatting). Ends
  with `ADVANCE → /land`, `ADVANCE → done`, or `HALT:`.
...
- **`committer`** — routine git plumbing (staging, commit messages, commit,
  ...
  Parent must not `git commit` itself.
...
- **`boba-watcher`** (cheap, escalate-capable) — read-only classifier for one
...
(`verifier`), hard debugging, and the `/watch-boba` cheap→strong carve-outs
...
as orchestrator; `boba-watcher` routine ticks stay cheap. Escalate individual
spawns to strong when `boba-watcher` returns `ESCALATE` (one re-classify) or when
drafting a scope/approach unblock — pass the strong model on that spawn only; do
not upgrade the whole loop. Claude Code switches via frontmatter `model:`; on
...
- **`/open-work [board]`** — browse the **unassigned, startable** items in a
  ...
  flagged ready/needs-you; research/spike picks may opt into `researcher` prep
  before `/dispatch` (never Mode-B auto-dispatch a spike). I pick numbers (no
  bare `go`/`all`), and behind a plan-preview gate ready items are handed to
  `/dispatch`.
