...
needs that tier but no pinned agent fits, pass the strong model on the spawn
(`model: "opus"` in Claude Code; `cursor-grok-4.5-high-fast` or `auto` in Cursor).
...
reasoning stays on `verifier` and explicit escalate paths. Pins today:

| Tier | Commands |
| --- | --- |
| cheap | `/ship-digest`, `/dispatch` |
| mid | `/my-work`, `/open-work`, `/start`, `/ship`, `/watch-boba`, `/land`, `/open-pr`, `/babysit-pr`, `/babysit-fleet`, `/review-requests`, `/address-reviews` |
