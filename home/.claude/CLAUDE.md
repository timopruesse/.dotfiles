...
frontmatter + `$ARGUMENTS`) and `home/.cursor/commands/` (Cursor: plain markdown
+ `$1`/`$2`/…, live-installed into `~/.cursor/commands/`). Shared protocols live
in `home/protocols/` (`HANDOFF-PROTOCOL.md`, `LOOP-PROTOCOL.md`) and resolve as
`~/protocols/…`.
