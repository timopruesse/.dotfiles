...
them by name when they match: {{PINNED_AGENTS}}.
...
`/watch-boba` may re-spawn it **once** with the strong pin (`{{STRONG_CURSOR}}`
...
1. Retry **once** with the same subagent but model `auto` (or omit `model` so it
   inherits / uses Auto).
2. If the retry also fails, report the error and follow the parent fallback in
   AGENT-ROUTING.md. Do not keep retrying or silently swap models.
