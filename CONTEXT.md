...
| **coding-agent resolve** | `home/.config/herdr/scripts/coding_agent_resolve.sh` — picks `claude` (work) vs `agy` (personal) for shared zsh/herdr/Neovim launchers (env → remote org → path), with Cursor `agent` available via override. Same work/personal split as git identity. |
...
| **coding-agent launch policy** | Shared keep-awake for `claude` / `agent` / `agy` in `coding_agent_policy.zsh` (sourced by `.zshrc` wrappers and `coding_agent_launch.sh`). Worktree isolation is opt-in via `-w` / `--worktree`. |
